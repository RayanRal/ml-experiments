...
# keep 5 largest contours
cnts = sorted(cnts, key=cv2.contourArea, reverse=True)[:5]

for c in cnts:
    # approximate the contour with @Approximates a polygonal curve(s) with the specified precision, with less vertices
...
warped = cv2.cvtColor(warped, code=cv2.COLOR_BGR2GRAY)
warped = threshold_adaptive(warped, block_size=251, offset=10)
